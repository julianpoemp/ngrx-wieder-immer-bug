...
function updateColumnWidth(state: TableState, innerWidth: number) {
    console.log('a');
    let newState = {
        ...state,
        table: {
            ...state.table,
            header: state.table.header.map((a) => ({
                ...a,
                style: {
    ...
    newState = {
        ...
        table: {
            ...newState.table,
            columnOptions: newState.table.columnOptions.map((a, index) => ({
                ...a,
                style: {
                    width: newState.table.header[index].style.width,
                },
            })),
        },
    };
    ...
    newState = {
        ...
        table: {
            ...newState.table,
            rows: newState.table.rows.map((a) => ({
                ...a,
                cells: a.cells.map((b) => ({
                    ...b,
                    style: {
                        ...b.style,
                        width: getColumnWidth(b, 'column', newState.table.header, innerWidth),
                    },
                })),
            })),
        },
        gui: {
            ...newState.gui,
            fullWidth: 40 + sum(newState.table.header.map((a) => a?.style?.width ?? 10)),
        },
    };
    console.log('d');
        ...
        on(TableActions.init.success, (state: TableState, { tools, projectRoles }) => {
            let newState: TableState = {
                ...
                table: {
                    ...state.table,
                    taskNumCounter: 1,
                    header: state.table.staticColumnTypes.map(
                        (a: any) =>
                            ({
                                type: 'header',
                                value: a,
            ...
            newState = updateContext(
                newState,
                'task_worker_username',
                newState.projectRoles.map((a) => ({
                    label: `${a.username} (${a.last_name}, ${a.first_name})`,
                    value: a.username ?? `DeletedUser_${a.account_id}`,
                    id: a.account_id,
                }))
            );
            ...
            newState = updateContext(
                newState,
                'task_config_label',
                newState.tools.map((a) => ({
                    label: `${a.name} - ${a.tool.name} ${a.tool.version}`,
                    value: a.name,
                    id: a.id,
                }))
            );
            const newRow = createRow(newState);
            ...
            newState = updateColumnWidth(newState, window.innerWidth);
            console.log('e');
            return newState;
        }),
